...
type archiveInputs struct {
	decision  domain.ValidityDecision
	packages  []domain.DataPackage
...
func (s *Service) loadArchiveInputs(id string) (archiveInputs, error) {
	d, err := s.repo.GetDecision(context.Background(), id)
	if err != nil {
		return archiveInputs{}, err
	}
	pk, err := s.repo.GetPackages(context.Background(), id)
	if err != nil {
		return archiveInputs{}, err
	}
	as, err := s.repo.GetAnomalies(context.Background(), id)
	if err != nil {
		return archiveInputs{}, err
	}
	ev, err := s.repo.GetEvidence(context.Background(), id)
	if err != nil {
		return archiveInputs{}, err
	}
	rv, err := s.repo.GetReviews(context.Background(), id)
	if err != nil {
		return archiveInputs{}, err
	}
	au, err := s.repo.GetAudit(context.Background(), id)
	if err != nil {
		return archiveInputs{}, err
	}
	return archiveInputs{decision: d, packages: pk, anomalies: as, evidence: ev, reviews: rv, audit: au}, nil
}
...
func (s *Service) Archive(ctx context.Context, id, actor, request string, expected int64) (Archive, error) {
	...
	inputs, e := s.loadArchiveInputs(id)
	if e != nil {
		return Archive{}, e
	}
	var out Archive
	err := s.repo.WithinTx(ctx, func(tx domain.Transaction) error {
		...
		out = Archive{TestRun: r, DataPackages: inputs.packages, Anomalies: inputs.anomalies, Evidence: inputs.evidence, Decision: inputs.decision, Reviews: inputs.reviews, Audit: inputs.audit}
		...
		if e = tx.UpdateDecisionArchive(ctx, inputs.decision.ID, out.ArchiveHash); e != nil {
			return e
		}
